...
export default function Schedule({ selectedLot }: ScheduleProps) {
  ...
  const { data: schedules, isLoading: isLoadingSchedules } = useSupabaseQuery(
    getLotSchedules(supabase, selectedLot.lot_id)
  );
  ...
  useEffect(() => {
    if (
      ...
      !filteredSchedules.find(
        (s) => s.schedule_id === selectedSchedule.schedule_id
      )
    ) {
    ...
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <ScheduleForm selectedLot={selectedLot} />
        </div>
      </div>
                  ...
                  <div className="flex items-center gap-4 justify-between">
                    <div className="flex flex-1 gap-12">
                      <div>
                        ...
                        <p>
                          {schedule.is_event
                            ? formatTime(schedule.event_start)
                            : schedule.start_time}
                        </p>
                      </div>
                      <div>
            ...
            <div>
                  ...
                  >
                    ...
                    <div className="flex justify-between items-center">
                      <div className="flex gap-2 items-center">
                        <p>
                          {selectedSchedule?.is_event &&
                            formatTime(selectedSchedule.event_start)}
                          {!selectedSchedule?.is_event &&
                        ...
                        <p>
                          {selectedSchedule?.is_event &&
                            formatTime(selectedSchedule.event_end)}
                          {!selectedSchedule?.is_event &&
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Trash2
                          className="w-4 h-4"
                          onClick={() =>
                            deletePriceTier({ price_id: tier.price_id })
                          }
                        />
                      </div>
                    </div>
              ...
              <div className="flex justify-center mt-8">
                {selectedSchedule?.schedule_id && (
                  <PriceTierForm sechuledId={selectedSchedule.schedule_id} />
                )}
              </div>
            </div>
          ) : selectedSchedule?.schedule_id ? (
